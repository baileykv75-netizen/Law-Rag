...
def job_audit_plan_path(job_id: UUID) -> Path:
    return _AtomicArtifactPath(job_output_dir(job_id) / "audit-plan.json")


def job_issue_legal_context_path(job_id: UUID) -> Path:
...
def job_issue_secondary_review_path(job_id: UUID) -> Path:
