...
def _load_inputs(job_id: UUID) -> tuple[DocumentInspection, OcrRunResult | None, bytes]:
    ...
    try:
        ...
        raise StructureProcessingError("Persisted document evidence is malformed and cannot be structured safely.") from exc
    ...
    if inspection.ocr_required_pages:
        ...
        try:
            ...
            raise StructureProcessingError("Persisted OCR evidence is malformed and cannot be structured safely.") from exc
...
def _native_units(page_text: str, page_number: int, evidence_id: str, order_start: int) -> list[EvidenceUnit]:
...
def build_evidence_stream(inspection: DocumentInspection, ocr_result: OcrRunResult | None) -> list[EvidenceUnit]:
    ...
    for page in inspection.pages:
        if page.route == PageRoute.NATIVE_TEXT_USABLE:
            page_units = _native_units(page.text, page.page_number, page.evidence_id, global_order)
...
def _build_clauses(units: list[EvidenceUnit], title_spans: set[tuple[int, str]]) -> tuple[list[Clause], list[UnnumberedBlock]]:
    ...
    for unit in units:
        if (unit.page_number, unit.text) in title_spans and current is None:
            continue
        parsed = _parse_clause_heading(unit.text)
        if parsed:
            flush_preamble()
...
def _extract_money(units: Iterable[EvidenceUnit]) -> list[MoneyMention]:
    ...
    multipliers = {"元": Decimal("1"), "百元": Decimal("100"), "千元": Decimal("1000"), "万元": Decimal("10000"), "亿元": Decimal("100000000")}
    for unit in units:
...
def _extract_references(units: Iterable[EvidenceUnit], clauses: list[Clause]) -> list[ReferenceMention]:
    ...
    for unit in units:
        ...
        heading = _parse_clause_heading(unit.text)
...
def _structured_candidates(units: Iterable[EvidenceUnit]) -> list[StructuredBlockCandidate]:
...
def _warnings(
    ...
    if not units:
        warnings.append(ExtractionWarning(warning_id="warning-0001", code="NO_TEXT_EVIDENCE", message="No usable text evidence was available for deterministic structuring."))
    if not titles:
        warnings.append(ExtractionWarning(warning_id=f"warning-{len(warnings) + 1:04d}", code="TITLE_UNRESOLVED", message="No conservative document title candidate was detected."))
    for party in parties:
        if party.resolution_state != ResolutionState.RESOLVED:
            span = party.source_spans[0]
            warnings.append(ExtractionWarning(warning_id=f"warning-{len(warnings) + 1:04d}", code="PARTY_NAME_UNRESOLVED", message=f"Party role {party.role_label} has no clear explicit name.", page_number=span.page_number, evidence_ids=span.evidence_ids))
    for mention in dates:
        if mention.resolution_state != ResolutionState.RESOLVED:
            span = mention.source_spans[0]
            warnings.append(ExtractionWarning(warning_id=f"warning-{len(warnings) + 1:04d}", code="INVALID_DATE", message=f"Explicit date could not be normalized safely: {mention.raw_text}", page_number=span.page_number, evidence_ids=span.evidence_ids))
    for reference in references:
        if reference.resolution_state != ResolutionState.RESOLVED:
            span = reference.source_spans[0]
            warnings.append(ExtractionWarning(warning_id=f"warning-{len(warnings) + 1:04d}", code="REFERENCE_UNRESOLVED", message=f"Reference target is not uniquely resolved: {reference.raw_text}", page_number=span.page_number, evidence_ids=span.evidence_ids))
    for unit in units:
        if unit.source_method == SourceMethod.OCR and (unit.confidence is None or unit.confidence < 0.85):
            warnings.append(ExtractionWarning(warning_id=f"warning-{len(warnings) + 1:04d}", code="LOW_CONFIDENCE_OCR_SOURCE", message="Canonical structure includes an OCR block that requires review.", page_number=unit.page_number, evidence_ids=unit.evidence_ids))
    return warnings


def build_contract_structure(job_id: UUID) -> CanonicalContract:
    inspection, ocr_result, fingerprint_bytes = _load_inputs(job_id)
    units = build_evidence_stream(inspection, ocr_result)
    titles = _title_candidates(units)
    title_keys = {(span.page_number, title.text) for title in titles for span in title.source_spans}
    ...
    warnings = _warnings(units, titles, parties, dates, references)
    ...
    job_contract_path(job_id).write_text(
...
def load_contract_structure(job_id: UUID) -> CanonicalContract:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Canonical contract structure for job {job_id} has not been generated.")
    try:
        return CanonicalContract.model_validate_json(path.read_bytes())
    except Exception as exc:
        raise StructureProcessingError("Persisted canonical contract structure is malformed.") from exc
...
def structure_summary(contract: CanonicalContract) -> StructureSummary:
    return StructureSummary(
        ...
        unresolved_reference_count=sum(reference.resolution_state != ResolutionState.RESOLVED for reference in contract.references),
