...
def _decision_label(value: str) -> str:
    labels = {
        ...
        "REJECTED": "误报",
        "MODIFIED": "已修改",
        "NEEDS_LAWYER_REVIEW": "需律师复核",
        "NEEDS_MORE_REVIEW": "需律师复核",
    }
...
def _render_docx(report: AuditReportDocument, destination: Path) -> None:
    ...
    for key, value in (
        ...
        ("文本覆盖", "已完成" if report.planning_coverage_complete else "待确认"),
        ("处理决策", f"{report.human_review_resolved_required_count}/{report.human_review_required_count} 已处理"),
    ):
        cells = table.add_row().cells
        cells[0].text = key
        cells[1].text = str(value)

    document.add_heading("审查结论与依据", level=1)
    for index, issue in enumerate(report.issues, start=1):
        document.add_heading(f"{index}. {issue.topic}", level=2)
        document.add_paragraph(f"处理要求：{'需要形成处理决策' if issue.requires_human_review else '可按报告建议处理'}")
        document.add_paragraph("为什么审：")
        _docx_add_bullets(document, issue.why_review)
        document.add_paragraph("风险判断：")
        document.add_paragraph(issue.primary_title)
        document.add_paragraph(issue.primary_reasoning)
        document.add_paragraph(f"建议：{issue.primary_suggestion}")
        document.add_paragraph("争议复审：")
...
def _render_pdf(report: AuditReportDocument, destination: Path) -> None:
    ...
    story += [
        ...
        _pdf_paragraph(f"审计基准日：{report.as_of}　合同类型：{report.contract_type}", body),
        _pdf_paragraph(
            f"文本覆盖：{'已完成' if report.planning_coverage_complete else '待确认'}　处理决策：{report.human_review_resolved_required_count}/{report.human_review_required_count} 已处理",
            body,
        ),
        Spacer(1, 4 * mm),
    ]
    ...
    for index, issue in enumerate(report.issues, start=1):
        story.append(_pdf_paragraph(f"{index}. {issue.topic}", heading))
        story.append(_pdf_paragraph(
            f"处理要求：{'需要形成处理决策' if issue.requires_human_review else '可按报告建议处理'}",
            body,
        ))
        for value in issue.why_review:
            story.append(_pdf_paragraph(f"• {value}", body))
        story.append(_pdf_paragraph(f"风险判断：{issue.primary_title}", body))
        story.append(_pdf_paragraph(issue.primary_reasoning, body))
        story.append(_pdf_paragraph(f"建议：{issue.primary_suggestion}", body))
        story.append(_pdf_paragraph("争议复审：", body))
        story.append(_pdf_paragraph(issue.secondary_reasoning, body))
        story.append(_pdf_paragraph(f"复审建议：{issue.secondary_suggestion}", body))
        for reason in issue.comparison_reasons:
            story.append(_pdf_paragraph(f"复核说明：• {reason}", body))
        if issue.omission_title or issue.omission_reasoning:
        ...
        if issue.human_decision is None:
            story.append(_pdf_paragraph("处理决策：待处理。", body))
        else:
            decision = issue.human_decision
            story.append(_pdf_paragraph(f"处理决策：{_decision_label(decision.state)}", body))
            if decision.reviewer_note:
                story.append(_pdf_paragraph(f"处理备注：{decision.reviewer_note}", body))
        if index < len(report.issues):
            story.append(PageBreak())

    story.append(PageBreak())
    story.append(_pdf_paragraph("审查说明", heading))
    ...
    if values:
        ...
        story.append(_pdf_paragraph("—", body))
    story.append(_pdf_paragraph("本报告由 AI 辅助生成，需人工确认。", body))
